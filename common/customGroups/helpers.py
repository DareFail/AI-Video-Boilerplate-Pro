...
def get_default_group_from_request(request):
    if "group" in request.session:
        try:
            foundGroup = request.user.customGroups.filter(
    ...
    if request.user.customGroups.count():
        return request.user.customGroups.filter(main_service=request.get_host()).first()
    else:
        return None
...
def create_default_group_for_user(user, group_name="Group", host=None):
    ...
    main_service = host
    group = CustomGroup.objects.create(name=group_name, unique_code=unique_code, main_service=main_service)
